...
\title{Percolation: an anthropological physics platform for social harnessing} % For titles, only capitalize the first letter
...
\author{Renato Fabbri\affil{1}{University of S\~ao Paulo}
...
\begin{abstract}
Percolation is a python package for harnessing the social networks of the user.
It is based on the pilars of: social percolation, 
creation of audiovisual artifacts, resource recommendation, 
typologies and the generation of reports. The fields of complex networks and linked data give
...
\section{}
%------------------------------------------------
The Percolation package consists of routines for social harnessing and can be directly hacked mainly through four packages: Gmane, Participation, Social, MASS. The text ahead consists of the core features of each of these five python packages.
...
\begin{itemize}
    ...
    \item Facilities to output images and latex tables. 
\end{itemize}
...
\begin{itemize}
    ...
    \item Routines to raise ontologies from data, return OWL code and images. (This is a novel and pertinent method of making ontologies to fit data, a somewhat \emph{data-driven ontology elaboration method}).
    \item Analysis of participatory data through complex networks (interaction and friendship networks) and text mining (BoW, Kolmorov-Smirnov histogram comparison and entropy).
    \item Resource recommendation, with explicit routines and potential uses.
    \item Recommendation of resources by both absolute criteria (ranking of users, words, etc.) and by criteria relative to a reference resource (e.g. recommend users given a reference post).
...
The social package delivers routines to access usual social network platforms data, such as Facebook, Twitter, LinkedIn and IRC. Core features are:
\begin{itemize}
...
\begin{itemize}
    ...
    \item Exact handle of duration and frequency parameters.
    \item ADSR envelopes.
    \item Table lookup.
    ...
    \item Predefined synthesis methods for other packages (Gmane, Social, Participation, Percolation).
\end{itemize}
...
\begin{itemize}
    \item Ease percolation in social systems by processes such as collection and diffusion of information.
    \item Enable knowledge about the networked self.
    \item Make abstract animations from social data.
    \item Make music from social data to fit animation as soundtrack.
    ...
    \item Integration of resources through RDF data and OWL ontologies. 
    \item Routines for representing data from social networks (tweets, Facebook data, IRC logs, LinkedIn, Gmane lists) as RDF.
    \item Cross provenance resource recommendation, extending facilities from the Participation package.
